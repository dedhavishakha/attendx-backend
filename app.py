...
from database import db, init_db, seed_demo_data
import os
...
# Create tables on startup (FAST - no bcrypt)
with app.app_context():
    init_db(app)

# Flag to track if demo data has been seeded
_demo_seeded = False

@app.before_request
def before_request():
    """Seed demo data on first request (lazy initialization)"""
    global _demo_seeded
    if not _demo_seeded:
        with app.app_context():
            seed_demo_data(app)
